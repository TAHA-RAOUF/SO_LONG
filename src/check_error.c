...
/*   Updated: 2025/03/27 17:58:55 by moraouf          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_walls(t_list *list)
{
	...
	while (list->map[++i])
	{
		if (i == 0 || i == c_line - 1)
		{
			...
			while (list->map[i][++j])
				if (list->map[i][j] != '1')
					return (fun_error("wall error"), fun_free(NULL, list->map,
							list));
		}
...
void	check_name(char *str)
{
	...
	len = ft_strlen(str);
	if (len <= 4 || str[len - 1] != 'r' || str[len - 2] != 'e' || str[len
			- 3] != 'b' || str[len - 4] != '.' || str[len - 5] == '/')
	{
...
void	check_intru(t_list *list)
{
	...
	count = count_characters(list, 'E') + count_characters(list, 'P');
	if (count != 2)
		return (fun_error("Prbl Player/exit"), fun_free(NULL, list->map, list));
	count = count_characters(list, 'C');
	if (count < 1)
